...
def infer(model,
          ...
          tokens_length : TensorSpec(shape = (), dtype = 'int32', static = True)    = None,
          ...
          step_fn    = None,
          training   = False,
          enc_padding_mask  : TensorSpec(shape = (None, None)) = None,

          method    = 'greedy',
          max_length    : TensorSpec(shape = (), dtype = 'int32', static = True) = None,
          max_new_tokens    : TensorSpec(shape = (), dtype = 'int32', static = True) = None,

          temperature   : TensorSpec(shape = (), dtype = 'float')   = None,
          length_temperature    : TensorSpec(shape = (), dtype = 'float')   = None,
          logits_filter = None,
    ...
    with Timer('initialization'):
        ...
        state_length, init_length, max_steps, max_length = _get_inference_lengths(
            model, tokens, prefix, initial_state, max_length, max_new_tokens = max_new_tokens,
    ...
    return _inference_methods[method](
        model,
        tokens,
        InferenceOutput(
            tokens  = generated,
...
def infer_greedy(self,
    ...
    def body(inputs, outputs, loop_state, first_iter = False):
        ...
        scores      = outputs.scores + K.where(
            loop_state.finished, K.array(0, dtype = next_token_score.dtype), next_token_score[:, 0]
        )
...
def process_logits(scores,
    ...
    if logits_filter is not None:
        if callable(logits_filter):
            ...
            scores = mask_batch_tokens(scores, K.convert_to_tensor(logits_filter, 'int32'))
...
def _get_inference_lengths(self,
                           tokens   = None,
                           prefix   = None,
                           initial_state    = None,
    ...
    if initial_state and isinstance(initial_state, dict):
        from .transformers.transformer_arch import _get_state_step
    ...
    if tokens is not None:
        init_length += tokens_length if tokens_length is not None else K.shape(tokens)[1]
    elif prefix is None:
        init_length += 1
...
def mask_batch_tokens(logits    : TensorSpec(shape = (None, None), dtype = 'float'),
    ...
    indices = K.stack([
        K.repeat(K.arange(K.shape(logits)[0]), K.shape(indices)[0]),
        K.tile(indices, [K.shape(logits)[0]])
    ], axis = 1)
...
def mask_tokens(logits    : TensorSpec(shape = (None, None), dtype = 'float'),
    ...
    """ Equivalent to `logits[indices] = value` compatible with `tensorflow graph` """
    return K.scatter_update(
        logits, indices, K.full((K.shape(indices)[0], ), value, dtype = logits.dtype)
    )
