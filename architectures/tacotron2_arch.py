...
from .layers import CustomRNNDropoutCell, CustomEmbedding, ConcatEmbedding, ConcatMode, HParamsLSA, LocationSensitiveAttention
...
class Tacotron2(keras.Model):
    ...
    def prepare_for_xla(self, *, inputs, max_length = None, padding_multiple = 64, ** _):
        tokens = inputs[0] if isinstance(inputs, tuple) else inputs
        tokens = pad_to_multiple(
            tokens, padding_multiple, axis = 1, constant_values = self.pad_token
        )
        inputs = tokens if not isinstance(inputs, tuple) else (tokens, ) + inputs[1:]
        ...
        return {'inputs' : inputs, 'max_length' : max_length}
