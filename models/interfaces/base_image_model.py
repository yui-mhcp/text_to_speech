...
_supported_image_normalization  = (None, 'vgg', 'mobilenet', 'vggface', 'tanh')
...
class BaseImageModel(BaseModel):
    def _init_image(self, input_size, * args, image_normalization = None, ** kwargs):
    ...
    def get_image(self, filename):
        ...
        return load_image(filename, target_shape = self.input_size, dtype = tf.float32)
    ...
    def preprocess_image(self, image):
        if self.image_normalization is None:
            return image
        elif self.image_normalization == 'vgg':
            return tf.keras.applications.vgg.preprocess_input(image)
        elif self.image_normalization == 'mobilenet':
            return tf.keras.applications.mobilenet.preprocess_input(image)
        elif self.image_normalization == 'vggface':
            return image[...,::-1] - tf.reshape([91.4953, 103.8827, 131.0912], [1, 1, 1, 3]) / 255.
        elif self.image_normalization == 'tanh':
            return image * 2. - 1.
