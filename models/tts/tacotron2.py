...
class Tacotron2(BaseTextModel, BaseAudioModel):
    ...
    def predict(self,
        ...
        time_logger.start_timer('text cleaning')
        ...
        time_logger.stop_timer('text cleaning')
        
        ####################
        #  Inference loop  #
        ####################
        
        show_idx = post_process(results, 0, display, play, post_processing, rate = self.audio_rate)
            ...
            for (i, text, clean, data) in inputs:
                ...
                if save:
                    if save_mel and len(mel) > 0:
                        ...
                        infos['mel'] = _get_filename(predicted, key, 'mel', mel_dir, 'mel_{}.npy')
