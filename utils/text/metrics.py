...
from .cleaners import collapse_whitespace, remove_tokens, remove_punctuation, lowercase
...
def text_distance(x, y, method, ** kwargs):
    if method not in _distance_methods:
        raise ValueError("Unknown distance method !\n  Accepted : {}\n  Got : {}".format(
            list(_distance_methods.keys()), method
        ))

    return _distance_methods[method](x, y, ** kwargs)

def text_distance_method_wrapper(fn = None, name = None, is_similarity = False):
    def wrapper(fn):
        @functools.wraps(fn)
        def inner(y_true, y_pred, as_matrix = False, mode = None, ** kwargs):
            y_true, y_pred = _normalize(y_true), _normalize(y_pred)
            ...
            if _is_nested_list(y_true) or _is_nested_list(y_pred):
                ...
                return pad_batch([
                    inner(y_true_i, y_pred_i, as_matrix = as_matrix, mode = mode, ** kwargs)
                    for y_true_i, y_pred_i in zip(y_true, y_pred)
                ], pad_value = -1., dtype = 'float32')
                ...
                if not as_matrix:
                    ...
                    return np.array([
                        inner(y_true_i, y_pred_i, mode = mode, ** kwargs)
                        for y_true_i, y_pred_i in zip(y_true, y_pred)
                    ])
                return np.array([
                    inner(y_true_i, y_pred, mode = mode, ** kwargs) for y_true_i in y_true
                ])
                ...
                return np.array([
                    inner(y_true, y_pred_i, mode = mode, ** kwargs) for y_pred_i in y_pred
                ])
            
            res = fn(y_true, y_pred, ** kwargs)
            if ((mode == 'similarity' and not is_similarity)
                or (mode == 'distance' and is_similarity)):
                res = - np.array(res)
            return res
        
        key = name or fn.__name__.replace('_distance', '')
        text_distance.dispatch(inner, key)
...
@text_distance_method_wrapper(is_similarity = True, name = 'f1')
...
def _is_nested_list(data):
    return isinstance(data, (list, tuple)) and data and isinstance(data[0], (list, tuple))
